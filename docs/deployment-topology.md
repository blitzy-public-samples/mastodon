...
The three long-running application services declared in `docker-compose.yml` are listed below, one row each. Every value is read directly from the source files; no service, image, port, or network is inferred beyond what they define.
...
**Why the application ports bind to `127.0.0.1`:** all three services publish their ports on the loopback interface only, so none is reachable directly from the internet; inbound traffic must arrive through the reverse proxy documented in Section B `Source: docker-compose.yml:L73-L74` `Source: docker-compose.yml:L97-L98`. This is also why `sidekiq`, which publishes no port at all, can be driven only indirectly through the shared Redis queue `Source: docker-compose.yml:L103-L119`.

**Data tier (for completeness — not part of the application-service table above):** the stateful backends live exclusively on `internal_network`, so they are never exposed outside the compose network. `db` runs `postgres:14-alpine` `Source: docker-compose.yml:L7` on `internal_network` only `Source: docker-compose.yml:L9-L10`; `redis` runs `redis:7-alpine` `Source: docker-compose.yml:L20` on `internal_network` only `Source: docker-compose.yml:L21-L22`; an `es` (Elasticsearch) service is present in the file but **commented-out / optional** `Source: docker-compose.yml:L28-L58`. `internal_network` is declared `internal: true`, which is what keeps the data tier private `Source: docker-compose.yml:L140-L141`.

## B. Inbound Routing (nginx)

`dist/nginx.conf` is the reverse proxy that fronts the loopback-bound services from Section A. One row per `upstream` and `location` rule; the purpose column states *why* each rule exists rather than restating its directives.
...
| `:443` → `^~ /assets/`, `/avatars/`, `/emoji/`, `/headers/`, `/ocr/`, `/packs/`, `/sounds/` | Served by nginx with `Cache-Control: public, max-age=2419200, must-revalidate` | Offloads immutable, fingerprinted static assets onto nginx with a four-week cache so the Rails app never serves them. `Source: dist/nginx.conf:L76-L116` |
...
*The application tier (`web`, `streaming`, `sidekiq`) attaches to both `external_network` and `internal_network`, while the data tier (`db`, `redis`, optional `es`) is reachable only on `internal_network` (`internal: true`); the solid edges into `db`/`redis` follow each service's `depends_on` declarations, the dashed `es` node and edges reflect the commented-out / optional Elasticsearch service, and `sidekiq` carries no inbound proxy edge or published port because it is reachable only through the shared Redis queue. `Source: docker-compose.yml:L67-L69` `Source: docker-compose.yml:L91-L93` `Source: docker-compose.yml:L113-L115` `Source: docker-compose.yml:L138-L141` `Source: docker-compose.yml:L75-L77` `Source: docker-compose.yml:L99-L101` `Source: docker-compose.yml:L110-L112` `Source: docker-compose.yml:L28-L58` `Source: docker-compose.yml:L103-L119`*
