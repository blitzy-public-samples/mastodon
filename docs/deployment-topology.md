...
| `:443` → `^~ /assets/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/assets/` URL tree straight from nginx under a four-week `public, must-revalidate` cache, so the Rails app never has to serve these static files. `Source: dist/nginx.conf:L76-L80` |
| `:443` → `^~ /avatars/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/avatars/` URL tree from nginx under the same four-week `must-revalidate` cache, keeping this static path off the Rails app. `Source: dist/nginx.conf:L82-L86` |
| `:443` → `^~ /emoji/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/emoji/` URL tree from nginx under the four-week `must-revalidate` cache instead of proxying it to the Rails app. `Source: dist/nginx.conf:L88-L92` |
| `:443` → `^~ /headers/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/headers/` URL tree from nginx under the four-week `must-revalidate` cache instead of proxying it to the Rails app. `Source: dist/nginx.conf:L94-L98` |
| `:443` → `^~ /ocr/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/ocr/` URL tree from nginx under the four-week `must-revalidate` cache so the request never reaches the Rails app. `Source: dist/nginx.conf:L100-L104` |
| `:443` → `^~ /packs/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/packs/` URL tree from nginx under the four-week `must-revalidate` cache so the request never reaches the Rails app. `Source: dist/nginx.conf:L106-L110` |
| `:443` → `^~ /sounds/` | Served by nginx; `Cache-Control: public, max-age=2419200, must-revalidate` | Serves the `/sounds/` URL tree from nginx under the four-week `must-revalidate` cache so the request never reaches the Rails app. `Source: dist/nginx.conf:L112-L116` |
